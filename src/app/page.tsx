...
import { GlitchText, ScrambleText, TypewriterText } from '@/components/ui/GlitchText'

const HeroBackground = dynamic(
  () => import('@/components/3d/HeroBackground').then(mod => mod.HeroBackground),
...
function CyberMarquee({ children, direction = 'left' }: { children: string; direction?: 'left' | 'right' }) {
...
function ScreenGlitch() {
  const [glitchArea, setGlitchArea] = useState<{active: boolean; x: number; y: number; w: number; h: number; slices: Array<{id: number; offsetY: number; shiftX: number; skew: number; scaleX: number; chromaShift: number}>} | null>(null)
